...
int type_return(int *type, char symbol, char *str, int *i)
{
    int j[2];
...
int sub_lexer(t_lexer **lex, char *str, int *i)
{
    ...
    if (!new)
        return (1);
    if (str[*i] == '>')
        new->type = type_return(type_setter(LEX_REDIRIN, LEX_APPEND), '>', str, i);
    else if (str[*i] == '<')
        new->type = type_return(type_setter(LEX_REDIROUT, LEX_HERE), '<', str, i);
    else if (str[*i] == '(')
        new->type = LEX_OPEN;
    else if (str[*i] == ')')
        new->type = LEX_CLOSE;
...
/*int sub_lexer(t_lexer **lex, char *str, int *i)
{
    ...
    if (str[*i] == '>')
    {
        new->type = sub_lexer(type_setter(LEX_REDIRIN, LEX_APPEND), str, i);
    ...
    {
        new->type = sub_lexer(type_setter(LEX_REDIROUT, LEX_HERE), str, i);
...
int	type_check3(t_lexer *lex, int *i)
{
...
int	type_check2(t_lexer *lex, int *i)
{
...
void print_redircmd(t_redircmd *redir, int indent) {
    ...
    print_indent(indent + 1);
    printf("QUOTE: %d\n", redir->quote); // Added newline character
    print_indent(indent + 1);
    printf("Mode: %d\n", redir->mode);
    print_indent(indent + 1);
    printf("FD: %d\n", redir->fd);
