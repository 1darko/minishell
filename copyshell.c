...
void *add_new_exec(t_execcmd **cmd, char *arg, int token)
{
    t_cmd *exec;

    exec = execcmd();
    ((t_execcmd*)exec)->arg = arg;
    ((t_execcmd*)exec)->next = 0;
    if(token == 'q')
        ((t_execcmd*)exec)->quoted = 1;
    ((t_execcmd*)exec)->space = 1;
    if(!(*cmd)->arg)
        *cmd = (t_execcmd *)exec;
    else
    {
        while((*cmd)->next)
            *cmd = (t_execcmd *)(*cmd)->next;
        (*cmd)->next = exec;
    }
}
...
int parse_single_argument(t_execcmd *cmd, char **ps, char *es, int argc)
{
    ...
    if (token == 'a') 
    {
        add_new_exec(&cmd, ft_strdup_arg(ptr_arg, ptr_earg), token);
        // cmd->args[argc] = ft_strdup_arg(ptr_arg, ptr_earg);
        return (1);
    }
    else if (token == 'q')
    {
        add_new_exec(&cmd, ft_strdup_arg(ptr_arg + 1, ptr_earg - 1), token);
        return (1);
    } 
    else if (token != 0)
        problem("syntax");
